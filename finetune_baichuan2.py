...
def preprocess(
    ...
    #     print("eos的字符是：", tokenizer.decode(im_end_id))
    ...
    for i, source in enumerate(sources):
        input_id, target = [], []
        system = tokenizer("{}".format(system_message), add_special_tokens=False).input_ids
        ...
        for j, sentence in enumerate(source):
            role = sentence["from"]
            if role == 'user':
                _input_id = tokenizer("<reserved_106>{}<reserved_107>".format(sentence["value"]), add_special_tokens=False).input_ids
                # print(tokenizer.decode(_input_id))
                _target = [IGNORE_TOKEN_ID] * len(_input_id)
            elif role == 'assistant':
                _input_id = tokenizer("{}".format(sentence["value"]), add_special_tokens=False).input_ids + [tokenizer.eos_token_id]
                # print(tokenizer.decode(_input_id))
                _target = _input_id
            else:
                raise NotImplementedError
...
def train():
    ...
    is_chat_model = 'chat' in model_args.model_name_or_path.lower() or "instruct" in model_args.model_name_or_path.lower()
    if (
